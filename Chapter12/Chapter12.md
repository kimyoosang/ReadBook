...
- 이런 일을 다루는 수단으로는 타입 코드 필드가 이싿. 타입 코드는 프로그래밍 언어에 따라 열거형이나 심볼, 문자열, 숫자 등으로 표현하며, 외부 서비스가 제공하는 데이터를 다루려 할 때 딸려오는 일이 흔하다
- 타입 코드만으로도 특벼러히 불편한 상황은 별로 없지만 그 이사으이 무언가가 필요할 때가 있다. 여기서 '그 이상'이라 하면 바로 서브클래스를 가리킨다
