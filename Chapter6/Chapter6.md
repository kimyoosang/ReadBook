...
## <U>6.9 여려 함수를 클래스로 묶기</U>

### **배경**

- 여러 함수를 클래스롤 묶으면 이미 만들어진 함수들을 재구성할 때는 무론, 새로 만든 클랠스와 관련하여 높친 연산을 찾아서 새 클래스의 메서드롤 뽑아내는 데도 좋다
...
- [여러 함수를 클래스로 묶기](./Example/CombineFuctionsIntoClass.md)

## <U>6.19 여러 함수를 변환 함수로 묶기</U>

### **배경**

- 데이터를 받아서 여러 가지 정보를 도출하는 작업들을 한데로 모아두면 검색과 갱신을 일관된 장소에서 처리할 수 있고 로직 중복도 막을 수 있다
- 이런 방법으로 변환 함수를 사용하는 방법이 있다. 변환 함수는 원본 데이터를 입력받아서 필요한 정보를 모두 도출한 뒤, 각각을 출력 데이터의 필드에 넣어 반환한다. 이렇게 해두면 도출 과정을 검토할 일이 생겼을 때 변환 함수만 살펴보면 된다
- 이 대신 여러 함수를 클래스로 묶기를 사용해도 된다. 원본 데이터가 코드 안에서 갱신될 때는 클래스로 묶는 편이 낫다. 변환함수로 묶으면 가공한 데이터를 새로운 레코드에 저장하므로, 원본 데이터가 수정되면 일관성이 깨질 수 있기 때문이다

### **절차**

1. 변환할 레코드를 입력받아서 그 값을 그대로 변환하는 함수를 만든다(이 작업을 대체로 깊은 복사로 철이해야 한다. 변환 함수가 원본 레코드를 바꾸지 않는지 검사하는 테스트를 마련해두면 도움될 때가 많다)
2. 묶을 함수 중 하나를 골라서 본문 코드를 변환 함수로 옮기고, 처리 결과를 레코드에 새 필드로 기록한다. 그런 다음 클라리언트 코드가 이 필드를 사용하도록 수정한다
3. 테스트한다
4. 나머지 관련 함수도 위 과정에 따라 처리한다

### **예시**

- [여러 함수를 변환 함수로 묶기](./Example/CombineFunctionsIntoTransform.md)

## <U>6.11 단계 쪼개기</U>

### **배경**

- 모듈이 잘 분리되어 있다면 다른 모듈의 상세 내용은 전혀 기억하지 못해도 원하는 대로 수정을 끝마칠 수도 있다. 이렇게 분리하는 가장 편한 방법 하나는 동작을 연이은 두 단계로 쪼개는 것이다
- 가장 대표적인 예는 컴파일러다. 컴파일러는 기본적으로 어떤 텍스트를 입력 받아서 실행 가능한 형태로 변환한다
- 컴파일 작업은 여러 단계가 순차적으로 연결된 형태로 분리하면 좋다. 텍스트를 토큰화하고, 토큰을 파싱해서 구문트리를 만들고, 구문 트리를 변환하는 다양한 단계를 거친 다음, 마지막으로 목적 코드를 생성하는 식
- 각 단계는 자신만의 문제에 집중하기 때문에 나머지 단계에 관해서는 자세히 몰라도 이해할 수 있다
- 다른 단계로 볼 수 있는 코드 영역들이 마침 서로 다른 데이터와 함수를 사용한다면 단계 쪼개기에 적합하다는 뜻이다

### **절차**

1. 두 번째 단계에 해당하는 코드르 독립 함수로 추출한다
2. 테스트한다
3. 중간 데이터 구조를 만들ㄹ어서 앞에서 추출한 함수의 인수로 추가한다
4. 테스트한다
5. 추출한 두 번재 단계 함수의 매개변수를 하나씩 검토한다. 그중 첫 번째 단계에서 사용되는 것은 중간 데이터 구조로 옮긴다. 하나씩 옮길 때마다 테스트한다
6. 첫 번째 단계 코드를 함수로 추출하면서 중간 데이터 구조를 반환하도록 만든다

### **예시**

- [단계 쪼개기](./Example/SplitPhase.md)
